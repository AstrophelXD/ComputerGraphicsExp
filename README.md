...
- `exp3/`：实验三（样条曲线动画图形显示），当前仅实现 21 组及以后对应内容；
- `exp4/`：实验四（多面体观察投影及动画显示），当前仅实现 21 组及以后对应内容；
...
- 主题：样条曲线动画图形显示；
- 实现范围：仅 21 组及以后（B 样条曲线 + 控制点二维变换动画）；
- 关键点：三次均匀 B 样条采样绘制、定时器驱动循环重绘。

### 实验4（`exp4`）

- 主题：多面体的观察投影及动画显示；
- 实现范围：仅 21 组及以后（十二面体线框动画）；
- 关键点：顶点/面表建模、观察变换透视投影、定时旋转重绘。
